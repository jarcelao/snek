...
from battlesnake_env.rules import DOWN, LEFT, RIGHT, UP
...
def state_for_devin(*, me_body, opponent_body, food=(), me_health=100):
    return BoardState(
        ...
        tuple(food),
        (),
        (Snake("devin", tuple(me_body), me_health), Snake("other", tuple(opponent_body))),
    )
...
@pytest.mark.parametrize("kwargs", ({"max_depth": 0}, {"max_nodes": 0}, {"max_depth": True}))
...
def test_score_orders_terminal_results():
    winner = BoardState(7, 7, 1, (), (), (Snake("devin", (Point(1, 1),)), Snake("other", (Point(5, 5),), elimination="wall-collision")))
    tie = BoardState(7, 7, 1, (), (), (Snake("devin", (Point(1, 1),), elimination="wall-collision"), Snake("other", (Point(5, 5),), elimination="wall-collision")))
    loss = BoardState(7, 7, 1, (), (), (Snake("devin", (Point(1, 1),), elimination="wall-collision"), Snake("other", (Point(5, 5),))))

    assert _score(winner, "devin", 1) > _score(tie, "devin", 1) > _score(loss, "devin", 1)
