...
def _unoccupied(state: BoardState, include_food: bool = True) -> list[Point]:
    ...
    return [Point(x, y) for x in range(state.width) for y in range(state.height) if Point(x, y) not in occupied]


def initial_state(width: int, height: int, snake_ids: tuple[str, ...], rng: random.Random) -> BoardState:
    """Make the official standard-map opening position."""
    state = BoardState(width, height, 0, (), (), tuple(Snake(id, ()) for id in snake_ids))
    mn, mid, mx = 1, (width - 1) // 2, width - 2
    corners = _shuffle([Point(mn, mn), Point(mn, mx), Point(mx, mn), Point(mx, mx)], rng)
    cardinals = _shuffle([Point(mn, mid), Point(mid, mn), Point(mid, mx), Point(mx, mid)], rng)
    ...
    if len(snake_ids) <= len(starts):
        ...
        snakes = tuple(Snake(id, (candidates[i],) * 3) for i, id in enumerate(snake_ids))
    state = BoardState(width, height, 0, (), (), snakes)
    center = Point(mid, mid)
    food: list[Point] = []
    for snake in snakes:
        head = snake.head
        choices = []
        for point in (Point(head.x - 1, head.y - 1), Point(head.x - 1, head.y + 1), Point(head.x + 1, head.y - 1), Point(head.x + 1, head.y + 1)):
            away = ((point.x < head.x < center.x) or (center.x < head.x < point.x) or
                    (point.y < head.y < center.y) or (center.y < head.y < point.y))
            corner = point.x in (0, width - 1) and point.y in (0, height - 1)
            if (0 <= point.x < width and 0 <= point.y < height and away and not corner
                    and point != center and point not in food):
...
def _feed(snakes: tuple[Snake, ...], food: tuple[Point, ...]) -> tuple[tuple[Snake, ...], tuple[Point, ...]]:
...
def _eliminations(state: BoardState, snakes: tuple[Snake, ...]) -> tuple[Snake, ...]:
    ...
    for index, snake in enumerate(result):
        if not snake.alive:
            continue
        if snake.health <= 0:
            result[index] = _eliminate(snake, Elimination.OUT_OF_HEALTH, None, state.turn + 1)
        elif any(p.x < 0 or p.x >= state.width or p.y < 0 or p.y >= state.height for p in snake.body):
            result[index] = _eliminate(snake, Elimination.OUT_OF_BOUNDS, None, state.turn + 1)
    ...
    for snake in alive:
        ...
        other = next((item for item in bodies if item.id != snake.id and snake.head in item.body[1:]), None)
        if other:
            pending[snake.id] = (Elimination.COLLISION, other.id)
            continue
        other = next((item for item in bodies if item.id != snake.id and item.head == snake.head and len(snake.body) <= len(item.body)), None)
        if other:
            pending[snake.id] = (Elimination.HEAD_COLLISION, other.id)
    return tuple(_eliminate(snake, *pending[snake.id], state.turn + 1) if snake.id in pending else snake for snake in result)


def _spawn_food(state: BoardState, settings: Settings, rng: random.Random) -> tuple[Point, ...]:
    needed = max(0, settings.minimum_food - len(state.food))
    if needed == 0 and settings.food_spawn_chance > 0 and 100 - rng.randrange(100) < settings.food_spawn_chance:
        needed = 1
    available = _shuffle(_unoccupied(state, include_food=True), rng)
    return state.food + tuple(available[:needed])


def step(state: BoardState, moves: dict[str, int], settings: Settings, rng: random.Random) -> tuple[BoardState, bool]:
    ...
    if set(moves) != {snake.id for snake in alive} or any(action not in DELTAS for action in moves.values()):
        raise ValueError("provide one valid action for every living snake")
    moved = _move(state, moves)
    starved = tuple(Snake(s.id, s.body, s.health - 1, s.elimination, s.eliminated_on_turn, s.eliminated_by) if s.alive else s for s in moved)
    damaged = []
    ...
    advanced = BoardState(state.width, state.height, state.turn + 1, food, state.hazards, eliminated)
    return BoardState(advanced.width, advanced.height, advanced.turn, _spawn_food(advanced, settings, rng), advanced.hazards, advanced.snakes), False
